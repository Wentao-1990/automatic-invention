...
package_list <- c("optparse","DESeq2")
...
#      write.table(dat, file=opts$input, append = F, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = F, col.names = T)
	...
	rldMat <- assay(rld)
	dds <- DESeq(dds)
	dds_deseq <- counts(dds,normalized = TRUE)
	res <- results(dds, alpha=0.05)
	res <- res[order(res$pvalue),]
	summary(res)
	write.table(res, file=paste(opts$output, ".txt",sep=""),append = F, sep="\t")
	write.table(dds_deseq, file=paste(opts$output, "ddsdeseq.txt",sep=""),append = F, sep="\t")
	write.table(vsdMat, file=paste(opts$output, "vsdnormal.txt",sep=""),append = F, sep="\t")
	write.table(rldMat, file=paste(opts$output, "rlognormal.txt",sep=""),append = F, sep="\t")
}
